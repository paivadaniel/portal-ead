...
//só mostra os cursos
$query = $pdo->query("SELECT * FROM $tabela WHERE status = 'Aguardando' order by data desc");
...
e quando o curso não estiver pago oculta o link que chama a função aulas -->
        <td>
        <i class="fa fa-square text-danger" style="margin-right:3px"></i> {$nome_curso} <span class="{$classe_curso}">{$item_curso}</span>
        ...
        <!-- abertura aprovar matrícula -->

        <big><a href="#" onclick="aprovar('{$id_mat}', '{$nome_curso}', '{$subtotal}', '{$obs}')" title="Editar Dados"><i class="fa fa-check-square verde"></i></a></big>

        <!-- fechamento aprovar matrícula -->
...
<script type="text/javascript">
    ...
    function aprovar(id_mat, nome_curso, subtotal, obs) {

        $('#id_mat').val(id_mat);
        $('#nome_curso_matricula').text(nome_curso);
        $('#subtotal').val(subtotal);
        $('#obs').val(obs);

        $('#tituloModal').text('Aprovar Registro');
